    ...
    <main ref={containerRef} className="min-h-screen bg-black text-white selection:bg-white selection:text-black">
      ...
      <section className="pt-48 pb-12 px-10">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-20 flex flex-wrap items-center gap-4 vehicle-animate">
            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 mr-4">Popular Cities:</span>
            {popularCities.map(city => (
              <button 
                key={city} 
                onClick={() => setSearchInput(city)}
                className={`h-10 px-6 rounded-full border text-[10px] font-bold uppercase tracking-widest transition-all ${searchInput === city ? 'bg-white text-black border-white' : 'border-zinc-800 hover:bg-white hover:text-black'}`}
              >
                {city}
              </button>
            ))}
      ...
      <section className="py-10 bg-zinc-950 border-y border-zinc-900 overflow-hidden">
        <div className="flex items-center gap-24 animate-marquee whitespace-nowrap">
          {[...vehicleBrands, ...vehicleBrands].map((brand, i) => (
            <span key={i} className="text-3xl font-bold tracking-tighter text-zinc-700 hover:text-white transition-colors cursor-default uppercase">
              {brand}
            </span>
          ))}
        </div>
      ...
      <section className="pt-20 px-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
           <div className="inline-flex bg-zinc-900/50 p-1.5 rounded-2xl border border-zinc-800">
              <Link href="/properties">
                <button className="px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 hover:text-white transition-all">
                  Properties
                </button>
              </Link>
              <button className="px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] bg-white text-black shadow-xl">
                Vehicles
              </button>
           </div>
           ...
           <div className="flex gap-4">
             <Link href="/sell">
               <button className="h-14 px-10 bg-white text-black rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-zinc-200 transition-all flex items-center gap-3">
                 <Plus className="w-4 h-4" /> Post Your Vehicle
               </button>
             </Link>
             <Link href="/dashboard">
               <button className="h-14 px-10 border border-zinc-800 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-zinc-900 transition-all">
                 My Showroom
               </button>
             </Link>
           </div>
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8 mb-20">
            <div className="flex gap-3 overflow-x-auto no-scrollbar w-full lg:w-auto">
              {["All", "Exotic", "Family", "SUVs", "Modified", "Electric"].map((cat) => (
                <button 
                  ...
                  className={`px-8 py-3 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${activeCategory === cat ? 'bg-zinc-800 text-white border-zinc-700' : 'bg-transparent text-zinc-600 border-zinc-900 hover:border-zinc-700'}`}
                >
                  {cat}
                </button>
              ))}
            ...
            <div className="relative w-full lg:w-96 group">
              ...
              <input 
                ...
                className="w-full bg-zinc-900/50 border border-zinc-800 rounded-2xl py-4 pl-14 pr-6 text-sm outline-none focus:border-zinc-500 transition-all"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {loading ? (
              ...
              filteredListings.map((item) => (
                <Link key={item.id} href={`/listing/${item.id}`} className="group relative aspect-[4/5] rounded-[2.5rem] overflow-hidden bg-zinc-900 vehicle-animate border border-zinc-900 hover:border-zinc-700 transition-all block">
                  <Image 
                    src={item.image} 
                    alt={item.title} 
                    fill 
                  ...
                  <div className="absolute top-8 left-8 flex gap-2">
                    ...
                    <span className="bg-black/50 backdrop-blur-md text-white px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-tighter border border-white/10">
                      PK Registered
                    </span>
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-8 space-y-6">
                    <div className="space-y-2">
                      <h3 className="text-3xl font-bold tracking-tighter leading-tight line-clamp-1">{item.title}</h3>
                    ...
                    <div className="grid grid-cols-3 gap-4 py-5 border-y border-white/5">
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] font-bold">12,500</p>
                      </div>
                      <div className="space-y-1">
                        <Fuel className="w-4 h-4 text-zinc-500" />
                        <p className="text-[9px] font-black text-zinc-500 uppercase">Fuel</p>
                        <p className="text-[11px] font-bold">Petrol</p>
                      </div>
                      <div className="space-y-1">
                        <Zap className="w-4 h-4 text-zinc-500" />
                        <p className="text-[9px] font-black text-zinc-500 uppercase">Type</p>
                        <p className="text-[11px] font-bold">Automatic</p>
                      </div>
                    </div>

                    <div className="flex items-center justify-between pt-2">
                      <span className="text-2xl font-bold tracking-tight text-white">{item.price}</span>
                      <div className="h-12 px-6 rounded-full bg-white text-black text-[10px] font-black uppercase tracking-widest hover:bg-zinc-200 transition-all opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 duration-300 flex items-center justify-center">
                        View Details
                      </div>
                    </div>
                  </div>
      ...
      <section className="max-w-7xl mx-auto px-10 py-32">
        <div className="bg-zinc-900/50 rounded-[3rem] border border-zinc-800 p-16 md:p-24 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div className="space-y-10">
            ...
            <h2 className="text-6xl font-bold tracking-tighter leading-none">Pakistan&apos;s First <br /> Verified Showroom.</h2>
            ...
            <div className="flex gap-8">
               <div className="space-y-2">
                 ...
                 <p className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Secure Trade</p>
               </div>
               <div className="w-[1px] h-12 bg-zinc-800" />
               <div className="space-y-2">
                 <p className="text-3xl font-bold">Biometric</p>
                 <p className="text-[10px] font-black uppercase tracking-widest text-zinc-600">Verification</p>
               </div>
            </div>
          </div>
          <div className="relative aspect-video min-h-[300px] rounded-3xl overflow-hidden shadow-2xl">
            <Image 
